package nochump.util.zip {	
	...
	import nochump.util.extend.ZipDecrypt;
	/**
	...
	public class ZipFile {
		 ...
		 */
		public function getInput(entry:ZipEntry,password:String=null):ByteArray {			
			buf.position = locOffsetTable[entry.name] + ZipConstants.LOCHDR - 4;			
			var fileLen:int = buf.readUnsignedShort();
			var extdLen:int = buf.readUnsignedShort();
			buf.position += fileLen + extdLen;					
			...
			switch(entry.method) {
				...
				case ZipConstants.DEFLATED:				
					...
					if(password != null )											 
				         b1 = ZipDecrypt.decryptMessage(b1,entry,password);
		...
		private function readEntries():void {
			...
			for(var i:uint = 0; i < entryList.length; i++) {
				...
				// handle file comment（不读取文件注释直接忽略）
